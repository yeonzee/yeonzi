...
SELECT TO_CHAR(C.SALES_DATE, 'YYYY') AS YEAR, TO_NUMBER(TO_CHAR(C.SALES_DATE, 'MM')) AS MONTH, COUNT(DISTINCT C.USER_ID) AS PURCHASED_USERS, ROUND(COUNT(DISTINCT C.USER_ID) / (SELECT COUNT(*) FROM USER_INFO WHERE TO_CHAR(JOINED, 'YYYY') = 2021), 1) AS PURCHASED_RATIO
